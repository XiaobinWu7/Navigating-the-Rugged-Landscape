...
from Attacker_KNN import Attack_GGS_CGC_NGS
from Attacker_KNN import Attack_Default
...
if __name__ == '__main__':
    ...
    parser.add_argument('--strategy', default='Default', type=str, choices=['Default', 'GGS_CGC_NGU'], help='Attack strategy')
    ...
    parser.add_argument('--binary_max_steps', type=int, default=10, help='Number of binary search steps')
    ...
    parser.add_argument('--task_num', type=int, default=10, help='Calibration cycles K')
    ...
    parser.add_argument('--initial_const', type=float, default=1.0, help='Initial trade-off constant')
    ...
    parser.add_argument('--step_alpha', type=float, default=5.0, help='Step size for PGD-like attack')
    parser.add_argument('--mu', type=float, default=1.0, help='Momentum factor for MI-FGSM-like attack')

    # Loss Config
    parser.add_argument('--cls_loss_type', type=str, default='Margin', choices=['Margin', 'CE'], help='Classification loss type')
    parser.add_argument('--confidence', type=float, default=15.0, help='Confidence for margin-based attack')

    parser.add_argument('--dis_loss_type', type=str, default='KNN',
                        choices=['CD', 'L2', 'KNN', 'None'], help='Distance loss type')
    parser.add_argument('--dis_loss_weight', type=float, default=1.0, help='Distance loss weight')
    parser.add_argument('--is_cd_single_side', action='store_true', default=False, help='Use single-side Chamfer distance')

    # Evaluation Config
    parser.add_argument('--curv_loss_knn', type=int, default=16, help='KNN for curvature estimation')
    parser.add_argument('--hd_loss_weight', type=float, default=0.0, help='Hausdorff loss weight')
    parser.add_argument('--curv_loss_weight', type=float, default=0.0, help='Curvature loss weight')
    parser.add_argument('--uniform_loss_weight', type=float, default=0.0, help='Uniform loss weight')

    parser.add_argument('--knn_smoothing_loss_weight', type=float, default=5.0, help='KNN smoothing loss weight')
    ...
    parser.add_argument('--laplacian_loss_weight', type=float, default=0.0, help='Laplacian loss weight for mesh attack')
    parser.add_argument('--edge_loss_weight', type=float, default=0.0, help='Edge loss weight for mesh attack')

    # System & Log
    parser.add_argument('-j', '--num_workers', type=int, default=4, help='Number of data loading workers')
    parser.add_argument('--use_normals', action='store_true', default=False, help='Use normals')
    parser.add_argument('--use_cpu', action='store_true', default=False, help='Use CPU mode')

    parser.add_argument('--is_debug', action='store_true', default=False, help='Enable debug mode')
    parser.add_argument('--is_pro_grad', action='store_true', default=False, help='Project gradient')
    parser.add_argument('--is_real_offset', action='store_true', default=False, help='Use real offset projection')
    parser.add_argument('--is_save_normal', action='store_true', default=False, help='Save normal information')
    parser.add_argument('--is_low_memory', action='store_true', default=False, help='Use low-memory mode')
    ...
    parser.add_argument('--calculate_project_jitter_noise_iter', type=int, default=50, help='Jitter recomputation interval')
    parser.add_argument('--jitter_k', type=int, default=16, help='K for jitter estimation')
    parser.add_argument('--jitter_sigma', type=float, default=0.01, help='Jitter sigma')
    parser.add_argument('--jitter_clip', type=float, default=0.05, help='Jitter clip')
    ...
    # Mesh Opt
    parser.add_argument('--is_partial_var', action='store_true', default=False, help='Use partial variable optimization')
    parser.add_argument('--knn_range', type=int, default=3, help='KNN range for mesh optimization')
    parser.add_argument('--is_subsample_opt', action='store_true', default=False, help='Enable subsample optimization')
    parser.add_argument('--is_use_lr_scheduler', action='store_true', default=False, help='Enable learning rate scheduler')
