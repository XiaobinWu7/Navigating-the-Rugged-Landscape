...
def create_data_task_set(cfg, x, set_size):
    """
    :param x: (B, N, C)
    :param set_size: Number of augmented samples generated
    ...
    for search_step in range(cfg.binary_max_steps):
        ...
        for step in range(cfg.iter_max_steps):
            ...
            for ii in range(task_num):
                ...
                current_grad_l1 = current_grad_l1.view(sample_size, 1, 1)
                g_qry_t = qry_grad / (current_grad_l1 + 1e-8)

                g_sample_t_mean = g_sample_t.data.view(sample_size, b, 3, n).mean(0)
                offset.grad += g_sample_t_mean

                g_qry_t_mean = g_qry_t.data.view(sample_size, b, 3, n).mean(0)
                offset.grad += g_qry_t_mean
