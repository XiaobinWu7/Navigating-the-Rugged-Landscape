...
def _forward_step(net, pc_ori, input_curr_iter, normal_ori, ori_kappa, target, scale_const, cfg, targeted, dist_func, V):
    ...
    # output_lfc = None
    if (V is not None) & (b != 1):
        V_repeat = V.repeat(cfg.sample_size, 1, 1)

        # input_curr_iter: (B, 3, N), V: (B, N, N) -> coeffs: (B, 3, N)
        coeffs = torch.bmm(input_curr_iter, V_repeat)
        lfc = torch.bmm(coeffs[..., :cfg.low_pass], V_repeat[..., :cfg.low_pass].transpose(2, 1))
        output_lfc = net(lfc)
