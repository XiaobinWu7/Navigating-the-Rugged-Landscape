...
def create_data_task_set(cfg, x, set_size):
    """
    :param x: (B, N, C)
    :param set_size: Number of augmentation samples to generate
    :return: (set_size*B, N, C)
    """
    B, N, C = x.shape

    x_expanded = x.unsqueeze(0).expand(set_size, -1, -1, -1).clone().view(set_size * B, N, C)
    noise = torch.randn_like(x_expanded) * cfg.gaus_noise
...
def attack(net, input_data, cfg, i, loader_len, saved_dir=None):
    ...
    y_repeat = gt_target.unsqueeze(0).repeat(sample_size, 1)
    y_expand = torch.cat([y_repeat[:, ii] for ii in range(b)])
    pc_ori_repeat = pc_ori.repeat(sample_size, 1, 1)
    normal_ori_repeat = normal_ori.repeat(sample_size, 1, 1)

    if cfg.curv_loss_weight !=0:
        kappa_ori = _get_kappa_ori(pc_ori_repeat, normal_ori_repeat, cfg.curv_loss_knn)
    else:
        kappa_ori = None
    ...
    for search_step in range(cfg.binary_max_steps):
        ...
        for step in range(cfg.iter_max_steps):
            if step == 0:
                ...
                offset.grad = torch.zeros_like(offset)

                sample_delta = torch.zeros(sample_size, 3, n).cuda()
                sample_delta.requires_grad_()

                qry_delta = torch.zeros(sample_size, 3, n).cuda()
                qry_delta.requires_grad_()
                periodical_pc = pc_ori.clone()

            input_all = periodical_pc + offset

            sample_data = create_data_task_set(
                cfg,
                (pc_ori + offset).data,
            ...
            with torch.no_grad():
                for k in range(b):
                    if input_curr_iter.size(2) < input_all.size(2):
                        ...
                        if cfg.log_dir == "dgcnn" or cfg.log_dir == "pointmlp" or cfg.log_dir == "pointmlp-elite":
            ...
            for ii in range(task_num):
                select_idx = torch.as_tensor([
                    np.random.choice(range(bi * sample_size, (bi + 1) * sample_size), sample_size, replace=False)
                    for bi in range(b)
                ])
                select_idx = select_idx.view(-1)
                sample_data = sample_data.reshape(-1, 3, 1024)
                sample_batch_x = sample_data[select_idx]
                if ii == 0:
                    '''PRIORS'''
                    _, normal_curr_iter, sample_loss, sample_loss_n, cls_loss, dis_loss, hd_loss, nor_loss, constrain_loss, info = _forward_step(
                        net,
                        pc_ori_repeat,
                        sample_batch_x.data + sample_delta,
                        normal_ori_repeat,
                        kappa_ori,
                        y_expand,
                        scale_const,
                    ...
                    sample_grad = sample_delta.grad

                    current_grad_l1 = torch.norm(sample_grad.view(sample_size, -1), p=2, dim=1)  # [sample_size, b]
                    current_grad_l1 = current_grad_l1.view(sample_size, 1, 1)  # 用于广播
                    g_sample_t = sample_grad / (current_grad_l1 + 1e-8)
                else:
                    g_sample_t = grad_record.reshape(-1, 3, n)
                    current_grad_l1 = torch.norm(g_sample_t.view(g_sample_t.shape[0], -1), p=2, dim=1)
                    current_grad_l1 = current_grad_l1.view(g_sample_t.shape[0], 1, 1)  # 用于广播
                    g_sample_t = g_sample_t / (current_grad_l1 + 1e-8)

                with torch.no_grad():
                    sample_delta -= step_a * g_sample_t

                '''POST'''
                _, normal_curr_iter, qry_loss, qry_loss_n, cls_loss, dis_loss, hd_loss, nor_loss, constrain_loss, info = _forward_step(
                    net,
                    pc_ori_repeat,
                    sample_batch_x.data + sample_delta.data + qry_delta,
                    normal_ori_repeat,
                    kappa_ori,
                    y_expand,
                    scale_const,
                ...
                qry_loss.backward()
                qry_grad = qry_delta.grad

                current_grad_l1 = torch.norm(qry_grad.view(sample_size, -1), p=2, dim=1)  # [sample_size, b]
                current_grad_l1 = current_grad_l1.view(sample_size, 1, 1)  # 用于广播
                g_qry_t = qry_grad / (current_grad_l1 + 1e-8)

                grad_record = (g_sample_t.data.view(b, sample_size, 3, n) + cfg.tau * g_qry_t.data.view(b, sample_size, 3,n))

                sample_delta.data.zero_()
                sample_delta.grad.data.zero_()
                qry_delta.data.zero_()
                qry_delta.grad.data.zero_()

            _, normal_curr_iter, loss, loss_n, cls_loss, dis_loss, hd_loss, nor_loss, constrain_loss, info = _forward_step(
                net, pc_ori, input_curr_iter, normal_ori, kappa_ori, target, scale_const, cfg, targeted, dist_func)

            current_dist = torch.norm(offset.view(b, -1), p=2, dim=1)
            all_loss_list[step] = current_dist.detach().tolist()
            g_final = grad_record.mean(1)
            grad_update = g_final / (torch.norm(g_final.view(b, -1), p=2, dim=1).view(b, 1, 1) + 1e-8)

            with torch.no_grad():
                offset -= cfg.lr * grad_update
            ...
            if cfg.is_debug:
                info = '[{5}/{6}][{0}/{1}][{2}/{3}] \t loss: {4:6.4f}\t output:{7}\t'.format(search_step+1, cfg.binary_max_steps, step+1, cfg.iter_max_steps, (qry_loss+sample_loss).item(), i, loader_len, output_label) + info
            else:
                info = '[{5}/{6}][{0}/{1}][{2}/{3}] \t loss: {4:6.4f}\t'.format(search_step+1, cfg.binary_max_steps, step+1, cfg.iter_max_steps, (qry_loss+sample_loss).item(), i, loader_len) + info
    ...
    return best_attack, target, (np.array(best_loss) < 1e10), best_attack_step, all_loss_list
