...
terminal_size = shutil.get_terminal_size(fallback=(80, 20))  # 提供默认值
...
class SaveGradStats(object):
    def __init__(self, fsave, fname='grad_list.mat'):
        self.fsave = fsave
        self.fname = fname
        if not os.path.exists(self.fsave):
            os.makedirs(self.fsave)
    ...
    def record_grad_list(self, grad_list):
        """
        grad_list: list of Tensors of shape [1, D]
        We will stack into shape [T, D]
        """
        # 转为 [T, D] 的 ndarray
        grads = torch.cat(grad_list, dim=0).cpu().numpy()
        self.grad_list.append(grads)

    def save_grad_list(self):
        """
        保存为 grad_list.mat，内容是一个 list，每个元素是 shape (T, D)
        """
        for idx, grad in enumerate(self.grad_list):
            fpath = os.path.join(self.fsave, f'grad_list_{idx}.mat')
            sio.savemat(fpath, {"grad": grad})
